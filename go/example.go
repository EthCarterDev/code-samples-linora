...
// `GET /system/config`
func GetlinoraConfig() (SystemConfigResponse, error) {
	systemConfigUrl := fmt.Sprintf("%s/system/config", linora_HTTP_URL)
...
// Generate linora private key from Ethereum private key
func GeneratelinoraAccount(config SystemConfigResponse, ethPrivateKey string) (string, string, string) {
...
func PerformOnboarding(
	...
	// URL
	onboardingUrl := fmt.Sprintf("%s/onboarding", linora_HTTP_URL)
	...
	req.Header.Add("linora-ETHEREUM-ACCOUNT", ethAddress)
	req.Header.Add("linora-STARKNET-ACCOUNT", dexAccountAddress)
	req.Header.Add("linora-STARKNET-SIGNATURE", GetSignatureStr(r, s))
...
func GetJwtToken(
	...
	// URL
	authUrl := fmt.Sprintf("%s/auth", linora_HTTP_URL)
	...
	req.Header.Add("linora-STARKNET-ACCOUNT", dexAccountAddress)
	req.Header.Add("linora-STARKNET-SIGNATURE", GetSignatureStr(r, s))
	req.Header.Add("linora-TIMESTAMP", timestampStr)
	req.Header.Add("linora-SIGNATURE-EXPIRATION", expirationStr)
...
func GetOpenOrders(jwtToken string) []*Order {
	// URL
	ordersUrl := fmt.Sprintf("%s/orders", linora_HTTP_URL)
...
func main() {
	// Load linora config
	linoraConfig, _ := GetlinoraConfig()

	// Initialize Ethereum account
	ethPrivateKey, ethAddress := GetEthereumAccount()

	// Generate linora account from Ethereum private key
	dexPrivateKey, dexPublicKey, dexAccountAddress := GeneratelinoraAccount(linoraConfig, ethPrivateKey)
	...
	SubmitOrder(linoraConfig, dexAccountAddress, dexPrivateKey, jwtToken)
