...
# Transfer USDC from old linora account to new linora account
...
async def main(old_linora_account_private_key_hex, new_linora_account_private_key_hex) -> None:
    # Load linora config
    linora_config = await get_linora_config(linora_http_url)

    # Get linora account addresses
    old_linora_account_address = get_linora_account_address(
        linora_config, old_linora_account_private_key_hex
    )
    new_linora_account_address = get_linora_account_address(
        linora_config, new_linora_account_private_key_hex
    )
    print(f"OLD linora Account Address: {old_linora_account_address}")
    print(f"NEW linora Account Address: {new_linora_account_address}")

    old_account = get_account(
        old_linora_account_address, old_linora_account_private_key_hex, linora_config
    )
    new_account = get_account(
        new_linora_account_address, new_linora_account_private_key_hex, linora_config
    )

    # Remove transfer amount to transfer all available balance
    await paraclear_transfer(linora_config, old_account, new_account, transfer_amount=100.0)
...
if __name__ == "__main__":
    ...
    old_linora_account_private_key_hex = os.getenv("OLD_linora_ACCOUNT_PRIVATE_KEY", "")
    new_linora_account_private_key_hex = os.getenv("NEW_linora_ACCOUNT_PRIVATE_KEY", "")
    asyncio.run(main(old_linora_account_private_key_hex, new_linora_account_private_key_hex))
