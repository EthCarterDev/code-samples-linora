...
from .linora_api_utils import Order
...
async def sign_request(
    ...
    """
    _secret_key: bytes = linora_maker_secret_key.encode("utf-8")
...
async def create_rest_headers(
    ...
    linora RESToverHTTP requests.
    """
    # timestamp, signature = await sign_request(
    #     linora_maker_secret_key=linora_maker_secret_key,
    #     method=method,
    #     path=path,
    #     body=body
    #     )
    ...
    headers: Dict = {
        # 'linora-API-Timestamp': timestamp.decode('utf-8'),
        # 'linora-API-Signature': signature.decode('utf-8'),
        "Authorization": f"Bearer {linora_jwt}"
    }
...
async def get_open_orders(
    ...
    headers: Dict = await create_rest_headers(
        linora_jwt=linora_jwt,
        linora_maker_secret_key="",
        method=method,
        path=path,
        body="",
    )
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(linora_http_url + path, headers=headers) as response:
...
async def fetch_trades(linora_http_url: str, linora_jwt: str, market: str) -> None:
    """
    linora RESToverHTTP endpoint.
    [GET] /trades
    """
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            linora_http_url + path, headers=headers, params=params
        ) as response:
            status_code: int = response.status
...
def check_token_expiry(status_code: int, response: Dict) -> None:
    """
    Checks the response from the linora API
...
async def post_order_payload(linora_http_url: str, linora_jwt: str, payload: dict) -> dict:
    """
    linora RESToverHTTP endpoint.
    [POST] /orders
    """
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(
                linora_http_url + path, headers=headers, json=payload
            ) as response:
                status_code: int = response.status
...
async def delete_order_payload(linora_http_url: str, linora_jwt: str, order_id: str) -> bool:
    """
    linora RESToverHTTP endpoint.
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.delete(linora_http_url + path, headers=headers) as response:
                status_code: int = response.status
                response: Dict = await response.json(content_type=None)
                check_token_expiry(status_code=status_code, response=response)
...
async def send_heartbeat_id(websocket: websockets.WebSocketClientProtocol, id: int) -> None:
    """
    Sends a Heartbeat to keep the linora WebSocket connection alive.
    """
...
async def send_auth_id(
    websocket: websockets.WebSocketClientProtocol, linora_jwt: str, msg_id: str
) -> None:
    """
    Sends an authentication message to the linora WebSocket.
    """
...
def starknet_account(config: ApiConfig) -> Account:
    ...
    account = get_account(
        account_address=config.linora_account,
        account_key=config.linora_account_private_key,
        linora_config=config.linora_config,
    )
...
async def get_usdc_balance(config: ApiConfig) -> int:
    ...
    usdc_address = config.linora_config["bridged_tokens"][0]["l2_token_address"]
...
async def deposit_to_paraclear(config: ApiConfig, amount: int) -> None:
    paraclear_address = config.linora_config["paraclear_address"]
    account = starknet_account(config)
    ...
    usdc_address = config.linora_config["bridged_tokens"][0]["l2_token_address"]
    usdc_decimals = config.linora_config["bridged_tokens"][0]["decimals"]
...
async def get_jwt_token(
    ...
    token = ""
    chain = int_from_bytes(linora_config["starknet_chain_id"].encode())
    account = get_account(
        account_address=account_address, account_key=private_key, linora_config=linora_config
    )
    now = int(time.time())
    expiry = now + 24 * 60 * 60
    ...
    headers: Dict = {
        "linora-STARKNET-ACCOUNT": account_address,
        "linora-STARKNET-SIGNATURE": flatten_signature(sig),
        "linora-TIMESTAMP": str(now),
        "linora-SIGNATURE-EXPIRATION": str(expiry),
    }
    path: str = "/auth"
    logging.info(f"get_jwt_token path:{linora_http_url + path} headers:{headers}")
    async with aiohttp.ClientSession() as session:
        async with session.post(linora_http_url + path, headers=headers) as response:
            status_code: int = response.status
            response: Dict = await response.json()
            if status_code != 200:
...
async def onboarding(
    ...
    headers: Dict = {
        "linora-ETHEREUM-ACCOUNT": ethereum_account,
        "linora-STARKNET-ACCOUNT": account_address,
        "linora-STARKNET-SIGNATURE": flatten_signature(sig),
    }
    path: str = '/onboarding'
    ...
    logging.info(f"onboarding path:{linora_http_url + path} headers:{headers}")
    async with aiohttp.ClientSession() as session:
        async with session.post(linora_http_url + path, headers=headers, json=body) as response:
            status_code: int = response.status
            if status_code != 200:
...
def get_recovery_phrase(config: ApiConfig) -> str:
    if config.linora_environment == "local":
...
def generate_accounts(config: ApiConfig):
    ...
    eth_chain_id = int(config.linora_config['l1_chain_id'])
    ...
    config.linora_account_private_key = hex(private_key)
    proxy_class_hash = config.linora_config['paraclear_account_proxy_hash']
    account_class_hash = config.linora_config['paraclear_account_hash']
    ...
    config.linora_account = account_address
    print("config.linora_account: ", config.linora_account)
