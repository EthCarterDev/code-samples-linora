...
def build_auth_message(chainId: int, now: int, expiry: int) -> TypedData:
    message = {
        ...
        "domain": {"name": "linora", "chainId": hex(chainId), "version": "1"},
        "primaryType": "Request",
        "types": {
            "StarkNetDomain": [
                {"name": "name", "type": "felt"},
...
def build_stark_key_message(chain_id: int) -> TypedData:
    message = {
        "domain": {"name": "linora", "version": "1", "chainId": chain_id},
...
def get_linora_account_address(linora_config: Dict, linora_account_private_key_hex: str) -> str:
    linora_key_pair = KeyPair.from_private_key(hex_to_int(linora_account_private_key_hex))
    linora_account_address = get_acc_contract_address_and_call_data(
        linora_config['paraclear_account_proxy_hash'],
        linora_config['paraclear_account_hash'],
        hex(linora_key_pair.public_key),
    )
    return linora_account_address


def generate_linora_account(
    linora_config: Dict, eth_account_private_key_hex: str
) -> Tuple[str, str]:
    eth_chain_id = int(linora_config['l1_chain_id'])
    stark_key_msg = build_stark_key_message(eth_chain_id)
    linora_private_key = derive_stark_key_from_eth_key(stark_key_msg, eth_account_private_key_hex)
    linora_key_pair = KeyPair.from_private_key(linora_private_key)
    linora_account_private_key_hex = hex(linora_private_key)
    linora_account_address = get_acc_contract_address_and_call_data(
        linora_config['paraclear_account_proxy_hash'],
        linora_config['paraclear_account_hash'],
        hex(linora_key_pair.public_key),
    )
    return linora_account_address, linora_account_private_key_hex
...
def get_account(account_address: str, account_key: str, linora_config: dict):
    client = FullNodeClient(node_url=linora_config["starknet_fullnode_rpc_url"])
    key_pair = KeyPair.from_private_key(key=hex_to_int(account_key))
    chain = get_chain_id(linora_config["starknet_chain_id"])
    account = Account(
        client=client,
...
async def get_jwt_token(
    ...
    chain_id = int_from_bytes(linora_config["starknet_chain_id"].encode())
    account = get_account(account_address, private_key, linora_config)
    ...
    headers: Dict = {
        ...
        "linora-STARKNET-SIGNATURE": f'["{sig[0]}","{sig[1]}"]',
        "linora-TIMESTAMP": str(now),
        "linora-SIGNATURE-EXPIRATION": str(expiry),
    }

    url = linora_http_url + '/auth'
