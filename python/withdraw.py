...
async def main(eth_private_key_hex: str) -> None:
    ...
    # Starknet account
    account = get_account(
        linora_account_address, linora_account_private_key_hex, linora_config
    )
    ...
    client, withdraw_tx_hash = await withdraw_from_paraclear(
        eth_account.address, amount, linora_config, account
    )
    ...
    await withdraw_from_l1_bridge(eth_account.address, amount, linora_config, w3)
