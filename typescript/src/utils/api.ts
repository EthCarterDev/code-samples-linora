...
export async function onboardUser(config: SystemConfig, account: Account) {
  ...
  const headers = {
    ...
    "linora-ETHEREUM-ACCOUNT": account.ethereumAccount,
    "linora-STARKNET-ACCOUNT": account.address,
    "linora-STARKNET-SIGNATURE": signature,
    "linora-TIMESTAMP": timestamp,
  };
...
export async function authenticate(config: SystemConfig, account: Account) {
  ...
  const headers = {
    ...
    "linora-STARKNET-SIGNATURE": signature,
    "linora-TIMESTAMP": timestamp,
    "linora-SIGNATURE-EXPIRATION": expiration,
  };
